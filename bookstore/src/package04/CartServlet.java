...
import package04.CartInfo;
import package04.ConnDB;
...
public class CartServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String sql1="select 图书编号,书名,优惠价格,购买数量,金额,购买日期,图片地址  from 购物车表";
		ResultSet rs1=conn.executeQuery(sql1);	//查询全部商品信息
		List list1=new ArrayList();		
		try {
			while(rs1.next()){	
				CartInfo cart1=new CartInfo();	
				cart1.setGoodsCode(rs1.getString("图书编号"));
				cart1.setGoodsName(rs1.getString("书名"));
		...
		String sql2="select sum(优惠价格*购买数量) as 金额合计,sum((价格-优惠价格)*购买数量) as 节省费用  from 购物车表";
		...
		request.getRequestDispatcher("cart4-2.jsp").forward(request, response);	
	}
}
