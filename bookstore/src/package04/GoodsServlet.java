...
public class GoodsServlet extends HttpServlet {
	...
	if (operate.equals("add")){
		...
		sql="select 图书编号,价格,购买数量  from 购物车表 where 图书编号=?";
		try {
			...
			if (rs.next()){
				...
				sql = "update 购物车表  set 购买数量 = ?, 金额 = ? where 图书编号=?";
				pstat = conn.prepareStatement(sql);
				...
				sql="select 图书编号,书名,价格,优惠价格,图片地址 from 图书数据表 where 图书编号=?";
				pstat = conn.prepareStatement(sql);
				pstat.setString(1,goodsCode);
				rs = pstat.executeQuery();
				if (rs.next()){
					name = rs.getString("书名").trim();
					price = rs.getString("价格").trim();
				...
				//将所选购商品加入到购物车中
				sql = "insert into 购物车表(图书编号,书名,价格,优惠价格,购买数量,金额,购买日期,图片地址) values(?,?,?,?,?,?,?,?)";
				pstat = conn.prepareStatement(sql);
				pstat.setString(1,goodsCode);
	...
	if (operate.equals("del")){
		String id = request.getParameter("code");
		sql = "delete from 购物车表 where 图书编号=?";
		try {
			pstat = conn.prepareStatement(sql);
	...
	if (operate.equals("clear")){
		...
		try {
			...
			conn.close();		
			//重定向到购物车页面
			response.sendRedirect("cart4-2.jsp");
		} catch (SQLException e) {
			// TODO 自动生成的 catch 块
			e.printStackTrace();
		}
