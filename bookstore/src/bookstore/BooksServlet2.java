...
public class BooksServlet2 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String leixing =s1.getAttribute("keywords2").toString().trim();
		...
		if (leixing==null || leixing.isEmpty()) {	
			...
			sql="select * from 图书数据表 where 类型  like '%" + leixing + "%'";
		...
		if (leixing==null || leixing.isEmpty()) {	
		   ...
		   //sql="select count(*) from 图书数据表  where 图书名称  like '%" + name + "%' or 图书作者 like '%" + name + "%'";
			sql="select count(*) from 图书数据表  where 类型  like '%" + leixing + "%'";
