...
public class BooksServlet3 extends HttpServlet {
	private static final long serialVersionUID = 1L;
    public BooksServlet3() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String leixing =s1.getAttribute("keywords3").toString().trim();
		...
		if (leixing==null || leixing.isEmpty()) {	
			...
			sql="select * from 图书数据表 where 图书类型  like '%" + leixing + "%'";
		...
		if (leixing==null || leixing.isEmpty()) {	
		   ...
		   //sql="select count(*) from 图书数据表  where 图书名称  like '%" + name + "%' or 图书作者 like '%" + name + "%'";
			sql="select count(*) from 图书数据表  where 图书类型  like '%" + leixing + "%'";
		   request.setAttribute("searchName", leixing);
		}
		...
		request.getRequestDispatcher("booksSearch2.jsp").forward(request, response);		//重定向页面
	}
